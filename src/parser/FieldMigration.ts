...
export class FieldMigration {
  ...
  static validateNodeIds(scenario: ScenarioV2): boolean {
    ...
    if (scenario.cues) {
      for (const cue of scenario.cues) {
        if (cue.root) {
          this.validateNodeIdRecursive(cue.root, visitedIds, `cue[${cue.id}].root`);
        }
      }
    }
   ...
   */
  private static validateNodeIdRecursive(node: any, visitedIds: Set<string>, path: string): void {
    // ID 필수 검증
    ...
    if (node.children && Array.isArray(node.children)) {
      node.children.forEach((child: any, index: number) => {
        this.validateNodeIdRecursive(child, visitedIds, `${path}.children[${index}]`);
      });
    }
  }
    ...
    if (node.pluginChain && Array.isArray(node.pluginChain)) {
      node.pluginChain.forEach((plugin: any, index: number) => {
        if (plugin.time_offset) {
          this.validateTimeRange(plugin.time_offset, `${path}.pluginChain[${index}].time_offset`);
        }
      });
    }
