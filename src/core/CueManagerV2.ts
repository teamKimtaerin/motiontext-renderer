...
export type CueLifecycleListener = (_event: CueLifecycleEvent) => void;
...
export class CueManagerV2 {
  ...
  private logger = createLogger('CueManagerV2', null);
  ...
  setScenario(scenario: Scenario): void {
    if (scenario.version !== '2.0') {
      throw new Error(
        `CueManagerV2 only supports v2.0 scenarios, got version "${scenario.version}"`
      );
    }
    ...
    this.logger.debug('Scenario set:', {
  ...
  private mountCue(cue: Cue, isPreload: boolean): void {
    ...
    this.stats.peakMounted = Math.max(
      this.stats.peakMounted,
      this.stats.currentMounted
    );
    ...
    this.logger.debug(`Mounted cue: ${cue.id} (preload: ${isPreload})`);
  }
  ...
  private scheduleUnmount(cue: Cue, _currentTime: number): void {
    ...
    this.logger.debug(
      `Scheduled unmount: ${cue.id} in ${this.options.cleanupDelayMs}ms`
    );
  }
  ...
  private unmountCue(cueId: string): void {
    ...
    this.logger.debug(`Unmounted cue: ${cueId}`);
  }
  ...
  private calculateDomLifetime(cue: Cue): TimeRange {
    ...
    return [Math.max(0, start - preloadSec), end + cleanupSec];
  }
  ...
  private enforceMemoryLimits(): void {
    ...
    const toUnmount =
      sortedCues.length -
      (this.options.maxMountedCues - this.getActiveCueCount());
    ...
    if (this.options.debugMode && toUnmount > 0) {
      this.logger.debug(
        `Memory limit enforced: unmounted ${toUnmount} inactive cues`
      );
    }
  }
  ...
  private getActiveCueCount(): number {
    return Array.from(this.mountedCues.values()).filter(
      (mounted) => mounted.isActive
    ).length;
  }
  ...
  private ensureDomLifetimes(): void {
    ...
    if (this.options.debugMode && autoCalculated > 0) {
      this.logger.debug(
        `Auto-calculated domLifetime for ${autoCalculated} cues`
      );
    }
  }
  ...
  private notifyLifecycle(event: CueLifecycleEvent): void {
    for (const listener of this.lifecycleListeners) {
      try {
        ...
        // keep as warn to avoid flooding; surface via logger
        this.logger.warn(
          '[CueManagerV2] Lifecycle listener error:',
          error as any
        );
      }
    }
  ...
  private logDebugStats(): void {
    ...
    this.logger.debug(
      `Stats: ${this.stats.currentMounted} mounted (${activeCount} active), peak: ${this.stats.peakMounted}`
    );
  }
  ...
  dispose(): void {
    ...
    this.logger.debug('Disposed. Final stats:', this.stats);
  }
}
